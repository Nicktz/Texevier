...
#' @examples
#' \dontrun{
#' create_template_html(directory = tempdir(), template_name = "Write_Up",
#' build_project = TRUE, open_project = TRUE)
#' }
#'
#' @export

create_template_html <- function (directory = tempdir(), template_name, bib.file, build_project = TRUE, open_project = TRUE){
  ...
  dir.create(file.path(directory, template_name, "Template"), showWarnings = FALSE)
  ...
  cat(readLines(system.file("templates/harvard-stellenbosch-university.txt", package = "Texevier")), file=paste0(file.path(directory,  template_name, "Template"), "/harvard-stellenbosch-university.csl"), sep="\n")

  cat(readLines(system.file("templates/default-fonts-Texevier.css", package = "Texevier")), file=paste0(file.path(directory,  template_name, "Template"), "/default-fonts-Texevier.css"), sep="\n")
  cat(readLines(system.file("templates/default-page-Texevier.css", package = "Texevier")), file=paste0(file.path(directory,  template_name, "Template"), "/default-page-Texevier.css"), sep="\n")
  cat(readLines(system.file("templates/default-Texevier.css", package = "Texevier")), file=paste0(file.path(directory,  template_name, "Template"), "/default-Texevier.css"), sep="\n")
  cat(readLines(system.file("templates/paged-Texevier.html", package = "Texevier")), file=paste0(file.path(directory,  template_name, "Template"), "/paged-Texevier.html"), sep="\n")
  # Create Rmd template file:

  template_name <- gsub(".Rmd", "", template_name)
  cat( readLines(system.file("templates/html_write_up_template.txt", package = "Texevier")), file = paste0(file.path(directory, template_name), "/", template_name, ".Rmd"), sep="\n")
