...
#subtitle: "This will appear as header"
...
keywords: "Multivariate GARCH \\sep Kalman Filter \\sep Rolling window OLS" # Use \\sep to separate
...
Discussion of data should be thorough with a table of statistics and ideally a figure. E.g. as below:
...
To reference this plot, add a ``\\label'' after the caption in the chunk heading, as done above. Then reference the plot as such: As can be seen, figure \ref{Figure1} is excellent. The nice thing now is that it correctly numbers all your figures (and sections or tables) and will update if it moves. The links are also dynamic.
...
Here follows another nice figure:

```{r figure2, echo = FALSE, message = FALSE, warning =  FALSE, fig.align = 'center', fig.cap = "Caption Here \\label{lit}", fig.ext = 'png', fig.height = 3, fig.width = 6}
...
g <-
  ggplot(data = mpg, aes(class)) + # All data must be tbl_df()::: class(mtcars %>% tbl_df())
...
To reference calculations __in text__, _do this:_ From table \ref{tab1} we see the average value of mpg is `r mean(mtcars[1:5,]$mpg)` (how awesome is that?).
