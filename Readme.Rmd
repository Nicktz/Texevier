...
This package helps a user set up an Elsevier template for writing academic journal reports in Rmarkdown. The setup requires very few lines of code and will get you up and running in _a few seconds_ (provided you have all the relevent dependencies installed - see below), with a fully working and good-to-go template. 
...
Alternatively, you could install the full [MikTex](https://miktex.org/download) version.
...
This will automatically install all the needed files and components in the right places - allowing you to immediately focus on your paper. By setting `r build_project = TRUE` and `open_project = TRUE r`, a new project file will be created in your chosen folder location, and opened with a new environment.

<template_name.Rmd> file will be created in the directory provided, which contains all the needed notes on how to write an academic paper in R. See if all your tex installations are working by opening this file, and hitting the knit button.
